...
public class GestionAlumnos {
    ...
    public static void addClassroom() {
        ...
        if (!classroomExist(id)) {
            ...
            groups.replace(searchGroup(), new ClassRoom(id, squareMeter));
        } else {
    ...
    public static void modifyGroup() {
        ...
        switch (modifyOption) {
            case 1:
                for (Map.Entry<Group, ClassRoom> entry : groups.entrySet()) {
                    if (entry.getKey().getId() == (searchGroup().getId())) {
                        System.out.print("¿Que id desea introducir?\nId: ");
                        entry.getKey().setName(lector.nextLine());
                    }
                }
                break;
            case 2:
                for (Map.Entry<Group, ClassRoom> entry : groups.entrySet()) {
                    if (entry.getKey().getName().equals(searchGroup().getName())) {
                        System.out.print("¿Que nombre desea introducir?\nId: ");
                        entry.getKey().setName(lector.nextLine());
                    }
                }
            ...
            case 3:
                for (Map.Entry<Group, ClassRoom> entry : groups.entrySet()) {
                    if (entry.getValue().getId() == (searchClassroom().getId())) {
                        System.out.print("¿Que id de clase desea introducir?\nId: ");
                        entry.getKey().setName(lector.nextLine());
                    }
                }
                break;
    ...
    public static Group searchGroup() {
        int id;
        for (Map.Entry<Group, ClassRoom> entry : groups.entrySet()) {
            System.out.printf("%-5d" + "%-10s", entry.getKey().getId(), entry.getKey().getName());
        }
        System.out.print("\nEligue el id del grupo que desea: ");
        id = validarEntradaNumerica(lector.nextLine().trim());
        while (id == -1) {
    ...
    public static ClassRoom searchClassroom() {
        int id;
        for (Map.Entry<Group, ClassRoom> entry : groups.entrySet()) {
            System.out.printf("%-5d" + "%-10s:m/2", entry.getValue().getId(), entry.getValue().getSquareMeter());
        }
        System.out.print("\nEligue el id del grupo que desea: ");
        id = validarEntradaNumerica(lector.nextLine().trim());
        while (id == -1) {
    ...
    public static int validarEntradaNumerica(String entrada) {
        try {
